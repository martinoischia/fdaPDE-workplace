...
# Just for efficiency?
dof_mean  <- matrix(data = NA, nrow = 5, ncol = 3)
dof_var  <- matrix(data = NA, nrow = 5, ncol = 3)
...
for (k in 0:4){
    ...
    for (i in 1:50){
        mod2_info <- smooth.FEM.mixed(
            ...
            verbose=TRUE,
            DOF.stochastic.realizations =reali,
            FLAG_ITERATIVE = TRUE, max.steps = 1000, threshold = tol, threshold_residual = tol)
        dof_vec<-c(dof_vec,mod2_info$edf[1])
        dof_vec2<-c(dof_vec2, mod2_info$edf[2])
        dof_vec3<-c(dof_vec3,mod2_info$edf[3])
    }
    dof_var[k+1,1]=var(dof_vec)
    dof_var[k+1,2]=var(dof_vec2)
    dof_var[k+1,3]=var(dof_vec3)
    dof_mean[k+1,1]=mean(dof_vec)
    dof_mean[k+1,2]=mean(dof_vec2)
    dof_mean[k+1,3]=mean(dof_vec3)
}
dof_mean
dof_var
